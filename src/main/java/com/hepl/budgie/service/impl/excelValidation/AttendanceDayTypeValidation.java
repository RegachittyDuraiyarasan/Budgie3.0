...
public class AttendanceDayTypeValidation implements ExcelValidation {
    ...
    public Map<String, List<ValidationRule>> getValidationRules(List<HeaderList> headers) {
        ...
        for (HeaderList header : headers) {
            ...
            if ("Employee_ID".equalsIgnoreCase(header.getHeader())) {
                ...
                rules.add(new ValidationRule(value -> getActiveEmployees().contains(value.toString()), "Employee ID must exist in the active employees list."));
            }
